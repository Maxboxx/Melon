...
Ptr<Kiwi::Value> IntegerToBoolNode::Compile(Weak<Expression> operand, CompileInfo& info, bool includeType) const {
	...
	info.AddInstruction(new Kiwi::AssignInstruction(SymbolTable::Bool->KiwiType(), var->Copy(), new Kiwi::NotEqualExpression(value, new Kiwi::Integer(SymbolTable::Bool->KiwiType(), 0))));
	return var;
}
