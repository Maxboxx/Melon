...
NodePtr GuardParser::Parse(ParsingInfo& info) {
	...
	if (NodePtr node = ConditionParser::Parse(info)) {
		...
		Pointer<GuardNode> gn = new GuardNode(info.scopes, FileInfo(info.filename, info.Current().line, info.statementNumber));
		gn->cond = node;

		if (info.Current().type != TokenType::Else) {
			ErrorLog::Error(SyntaxError(SyntaxError::ExpectedAfterIn("'else'", "condition", "guard statement"), FileInfo(info.filename, info.Current(-1).line, info.statementNumber)));
