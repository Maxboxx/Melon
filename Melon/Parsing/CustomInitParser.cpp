...
NodePtr CustomInitParser::Parse(ParsingInfo& info) {
	...
	Pointer<CustomInitNode> cn = new CustomInitNode(info.scope->AbsoluteName(), FileInfo(info.filename, info.Current(-1).line, info.statementNumber, info.currentNamespace, info.includedNamespaces));
